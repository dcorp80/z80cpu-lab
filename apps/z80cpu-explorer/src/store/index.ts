...
export async function createAppStore(deps: CreateStoreDeps): Promise<Store> {
  ...
  // Hoist async backend reads ahead of createRoot — the wrapper is
  // synchronous, and every reactive (signal/store/memo) needs to land
  // inside one owner scope so `store.dispose()` can release them via
  // `rootDispose`. Hoisting also keeps Solid's owner-context semantics
  // simple: no awaits inside the root, so reactives created post-await
  // can't drop out of the owner.
  const loaded =
  ...
  // `createMemo` would log "computations created outside a `createRoot`
  // or `render` will never be disposed", and reactives would leak across
  // boot/dispose cycles in the same browser context (relevant for the
  // browser test tier where many bootApp/dispose pairs share a Chromium).
  let result!: Store;
  createRoot((rootDispose) => {
    ...
    const [ioWatchJumpVersionWrite, setIoWatchJumpVersionWrite] =
      createSignal(0);
    ...
    // Memoized: now that createAppStore runs inside `createRoot`, top-level
    // `createMemo` has a proper owner and disposes alongside the store. The
    // dedup is negligible for a single boolean comparison, but
    // `reloadSettingsDirty` ORs three of them and is read both inside JSX
    // (`<Show when={...}>`) and by the section's `isCollapseLocked` hook —
    // memoizing the OR keeps the section's collapse-lock evaluation a
    // single signal read.
    ...
    const [lastMemWrite, setLastMemWrite] =
      createSignal<BusAccessRecord | null>(null);
    ...
    function assertAddr16(addr: number, label: string): void {
      if (!Number.isInteger(addr) || addr < 0 || addr > 0xffff) {
        throw new RangeError(
          `${label}: address out of range 0..0xFFFF: ${addr}`,
        );
      }
    }
